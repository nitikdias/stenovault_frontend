...
export default function File() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await fetch('https://bc12-157-50-187-137.ngrok-free.app/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  try {
    const res = await fetch('https://bc12-157-50-187-137.ngrok-free.app/process', { method: 'POST' });
  ...
  try {
    ...
    const res = await fetch('https://bc12-157-50-187-137.ngrok-free.app/clear', { method: 'POST' });
    if (res.ok) {
